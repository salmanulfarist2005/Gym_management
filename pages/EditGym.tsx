...
const EditGym: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (gym) {
      setFormData({
        name: gym.name,
        email: `${gym.name.toLowerCase().replace(/\s/g, '')}@gym.com`,
        phone: '+1 (555) 000-0000',
        address: gym.location,
        description: `This is the main branch for ${gym.name}.`,
        isActive: gym.status === 'Active'
      });
    }
  }, [id]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
        setLoading(false);
        console.log('Gym Updated:', formData);
    ...
    <div className="space-y-6 max-w-2xl mx-auto pb-12">
        <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-gray-500 text-sm">Update branch details for {id}</p>
            </div>
        </div>
